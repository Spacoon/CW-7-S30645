...
public class DbService(IConfiguration config) : IDbService
{
    ...
    public async Task<IEnumerable<TripGetDTO>> GetTripsAsync()
    {
        ...
        // Pobiera wszystkie wycieczki z bazy danych oraz nazwe kraju.
    ...
    public async Task<IEnumerable<ClientTripGetDTO>> GetTripsByClientIdAsync(int id)
    {
        ...
        // Pobiera wszystkie wycieczki dla klienta o podanym id oraz nazwe kraju.
        ...
        if (!reader.HasRows)
        {
            throw new NotFoundException($"Client with id: {id} does not exist or has no trips");
        }
    ...
    {
        await using var connection = new SqlConnection(_connectionString);
        // Dodaje nowego klienta do bazy danych i zwraca wygenerowane Id
    ...
    public async Task RegisterClientTripAsync(int id, int tripId)
    {
        ...
        // Sprawdza czy klient o podanym id jest już zarejestrowany na wycieczkę o podanym tripId
        ...
        await using (var checkClientTripCommand = new SqlCommand(isClientRegisteredForTripSql, connection))
        {
            ...
            if (clientTripExists)
            {
                throw new ConflictException($"Client with id: {id} is already registered for trip with id: {tripId}");
            }
        }
        // Pobiera maksymalna liczbe osob, ktore mogą wziąć udział w wycieczce
        const string getMaxPeopleSql = "SELECT MaxPeople FROM Trip WHERE IdTrip = @tripId";
        // Pobiera liczbę osób aktualnie zarejestrowanych na wycieczkę
